...
public class WebApp(LogService logService, DownloaderService downloaderService) : AppBase(logService) {
    ...
    protected override async Task InstallCoreAsync(CancellationToken cancellationToken) {
        var download = await downloaderService.DownloadFileAsync(Url, Paths.DownloadPath, cancellationToken: cancellationToken) ?? throw new InvalidOperationException("Failed to download the executable.");
