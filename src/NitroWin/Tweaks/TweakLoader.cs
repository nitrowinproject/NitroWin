...
internal static class TweakLoader {
    ...
    private static async Task DownloadTweaksAsync() {
        var tweaksArchive = await FileDownloader.DownloadFileAsync(Globals.Config.Options.TweakUrl, Globals.DownloadFolder) ?? throw new NullReferenceException();
    ...
    private static async Task<List<Tweak>> ParseTweaksAsync() {
        ...
        foreach (var file in Directory.EnumerateFiles(Path.Join(TweakPath, "Tweaks-3", "Tweaks"), "*.yml", SearchOption.AllDirectories)) {
            try {
